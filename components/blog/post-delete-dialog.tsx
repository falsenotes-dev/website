...
import { handleDelete, handleDeleteDraft } from "../delete";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
      ...
      <AlertDialogContent className="flex flex-col justify-center !w-96 !rounded-lg">
        ...
        <AlertDialogFooter className="!flex-row !justify-center space-x-2">
          ...
          <AlertDialogAction onClick={
            async () => {
              await handleDeleteDraft(post?.id)
            }
          } className={buttonVariants({ variant: 'destructive' })}>Delete Draft</AlertDialogAction>
          <AlertDialogAction onClick={
            async () => {
              const res = await handleDelete(post?.id)
              await validate(`/@${user?.username}`)
              if (res.status === 204) {
                router.push(`/`)
              }
            }
          } className={buttonVariants({ variant: 'destructive' })}>Delete Post</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
