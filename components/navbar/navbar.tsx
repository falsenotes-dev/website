...
function Navbar({ notifications, searchHistory }: { notifications: any, searchHistory: UserSearchHistory[] }) {
  ...
  return (
    <div className="max-w-[1140px] min-w-[280px] mx-auto w-full sticky top-4 z-20">
            ...
            status == 'authenticated' ? (
              <>
                ...
                <Button variant={"ghost"} size={"icon"} className="h-10 w-10 relative hover:shadow-md hover:!bg-accent/10 hover:backdrop-blur-md" onClick={() => router.replace('/notifications')}>
                  ...
                  {notifications > 0 && (<Badge className="left-5 font-normal px-1.5 py-0 absolute top-0 h-5 min-w-[1.25rem] max-w-max" >{numberFormat(notifications)}</Badge>)}
                </Button>
                <UserNav />
              </>
            ) : (
